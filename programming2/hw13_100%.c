...
int main(void)
{
    ...
    int act_style,dir=4,i,n=0,snake_len=1;    //snake_len=-1代表結束
    initialize(game_map);
    ...
    while (1)
    {
        ...
        else if(act_style==5) snake_len=act_move(game_map,snake,dir,snake_len);
        else if(act_style==0) act_print(snake_len,snake);
        else dir=act_style;
        if(snake_len==-1) break;
        /*scanf("%d",&act_style[n]);
        n++;
...
int act_move(int map[][20],int snake[][2],int dir,int snake_len)
{
    ...
    thing=map[newY][newX];
    if(thing==0 || thing==-1 || thing==snake_len)
    {
